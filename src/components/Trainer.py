...
class Trainer:
    def __init__(self, epoch = 20):
        ...
        self.data_dir = os.path.join(PROJECT_ROOT, self.config["data_ingestion"]["extract_path"], "ModelNet10")
    ...
    def process(self):
        ...
        for eph in range(self.epoch):
            ...
            logger.info(f"Epoch {eph+1}: Train Acc: {train_acc:.4f}, Test Acc: {test_acc:.4f}")
